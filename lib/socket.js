...
module.exports = function(socket, connections) {
  ...
  socket.on('connect', function(data) {
    ...
    if(connection === undefined) {
      connection = new IRCLink(config.irc.hostname, data.port, data.secure, data.selfSigned, data.nick, data.realName, data.password, data.rejoin, data.away);
      ...
      socket.emit('restore_connection', {nick: connection.client.nick,
        server: config.irc.hostname, channels: connection.client.chans});
    ...
    socket.on('nick', function(data){
      ...
      connection.client.opt.nick = connection.client.nick;
    });
